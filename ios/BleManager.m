...
{
    return @[@"BleManagerDidUpdateValueForCharacteristic", @"BleManagerStopScan", @"BleManagerDiscoverPeripheral", @"BleManagerConnectPeripheral", @"BleManagerDisconnectPeripheral", @"BleManagerDidUpdateState",@"iosBleManagerDidUpdateValueForCharacteristic"];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if (readCallback != NULL) {
        ...
        if (hasListeners) {
//            NSArray *tempArr = ([characteristic.value length] > 0) ? [characteristic.value toArray] : [NSNull null];
//            NSLog(@"peripheral==%@",[characteristic.value hexadecimalString]);
            
            [self sendEventWithName:@"iosBleManagerDidUpdateValueForCharacteristic" body:@{@"peripheral": peripheral.uuidAsString, @"characteristic":characteristic.UUID.UUIDString, @"service":characteristic.service.UUID.UUIDString, @"value": ([characteristic.value length] > 0) ? [characteristic.value hexadecimalString] : [NSNull null]}];
        }
    }
}
